...
import CardOrderSkeleton from '../Skeleton/CardOrderSkeleton'
import CardTransaksi from '../Card/Transaksi/CardTransaksi'

const ListOrder = ({ id, token }) => {
    ...
    return (
        <div className='block space-y-4 p-4 h-full overflow-hidden'>
            ...
            <div className='w-full h-full flex flex-col gap-4 items-center justify-start'>
                {
                    isLoading ? Array.from({ length: 5 }).map((_, i) => (<CardOrderSkeleton key={i} />)) : listPemesanan?.data.data.order.length > 0 ? listPemesanan?.data.data.order.map((item, index) => (
